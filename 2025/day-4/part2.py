...
class Matrix:
    ...
    def free_rolls(self, coordinates: list[tuple[int, int]]) -> None:
        for row, column in coordinates:
            if self.grid[row][column] == 0:
                raise ValueError(f"Coordinates ({row}, {column}) is already free.")
    ...
    for row in range(matrix.total_row_count):
        for col in range(matrix.total_column_count):
            ...
            if count < limit:
                ...
                free_places.append((row, col))

    matrix.free_rolls(free_places)
...
def solve(data: list[str]) -> int:
    ...
    total_results = accessible_rolls_via_forklift(matrix)
    passes = 0
    passes_limit = 100
    while True:
        passes += 1
        if passes > passes_limit:
            print(f"Breaking due to limit of {passes_limit} passes is reached.")
            break
        result = accessible_rolls_via_forklift(matrix)
        if result == 0:
            break
